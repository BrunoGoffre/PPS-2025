...
export class DataService {
  ...
  login(usuario: Usuario): Promise<any> {
    return new Promise<any>((resolve, reject) => {
      ...
      this.firebaseAuth
        .signInWithEmailAndPassword(usuario.email, usuario.pass || '')
        .then(
          (response) => {
            if (!response.user) {
  ...
  public async fetchQR(codigoQR: string) {
    const db = getFirestore(); // si no lo has inicializado aún
    const docRef = doc(db, 'codigos', codigoQR);
  ...
  public async getUserInfo(id: string) {
    ...
    return (await snapshot.exists()) ? (snapshot.data() as any) : null;
  }
  ...
  async obtenerTop5(nivel: string): Promise<any[]> {
    const puntajeRef = collection(this.firestore, 'puntajes');
    const q = query(
      puntajeRef,
      where('nivel', '==', nivel),
      orderBy('tiempo'),
