...
export class ImagenService {
  ...
  public static fotosUsuarioFeas: Imagen[] = [];
  public static fotosUsuarioLindas: Imagen[] = [];
  ...
  async sacarFoto(usuario: Usuario, tipo: TipoImagen): Promise<Imagen> {
    ...
    try {
      ...
      if (tipo === TipoImagen.POSITIVA) {
        ImagenService.fotosBonitas.unshift(imagen);
      } else {
        ImagenService.fotosFeas.unshift(imagen);
      }
    } catch (error) {
  ...
  fetchUsuarioFeas(id: string) {
    ImagenService.fotosUsuarioFeas =
      ImagenService.imagenes
        .filter((img) => img.usuario === id && img.tipo === TipoImagen.NEGATIVA)
        .sort((a, b) => this.comparadorFechas(a, b)) ?? [];
  }
  fetchUsuarioLindas(id: string) {
    ImagenService.fotosUsuarioLindas =
      ImagenService.imagenes
        .filter((img) => img.usuario === id && img.tipo === TipoImagen.POSITIVA)
        .sort((a, b) => this.comparadorFechas(a, b)) ?? [];
  }
